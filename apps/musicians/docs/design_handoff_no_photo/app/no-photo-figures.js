/* Jazzlore Musicians · No-photo treatment · figure library.
 *
 * Renders an editorial line-drawing of a musician in the act of playing,
 * keyed off the primary-instrument string. Used inside .duo3 when no
 * photograph is available — replaces the corner-initials fallback.
 *
 * Exports to window:
 *   - FIG_LIB           : { instrument-key → SVG inner markup }
 *   - figKey(inst)      : maps free-form instrument string → key in FIG_LIB
 *   - serialFor(name)   : stable 3-digit serial derived from a name
 *   - NoPhotoMark(props): React component {inst} → <div.duo3-mark><svg>…</svg></div>
 *
 * Drawings: single-weight strokes, viewBox 100×140. Stroke weight is set
 * via CSS (.duo3-mark svg .ln { stroke-width }) so each .mu3 context
 * (conn 64px, era-tile 140px, etc.) can tune line weight independently.
 *
 * Adding an instrument: append a key to FIG_LIB, add a branch to figKey.
 */

(function () {
  // ─────────── Figure markup (inner SVG) ───────────
  // All figures share a common scaffold:
  //   - head: ellipse rx≈6.5 ry≈7.5 near top
  //   - body sides: two curving paths
  //   - legs: two curving paths, ending in foot ticks at y≈134
  //   - instrument: rendered in place
  // The .ln class gets stroke from CSS. .dot class is filled. .keyfill is a
  // semi-transparent fill used for piano-key surfaces.

  const FIG_LIB = {
    // PIANO — pianist seated profile, both arms reaching forward to keys.
    piano: `
    ...
    // TRUMPET — standing, head tilted back, horn extending up-right.
    trumpet: `
    ...
    // TROMBONE — standing facing right; slide extends forward, bell behind.
    trombone: `
    ...
    // SAX — standing, sax body J-curves down across torso.
    sax: `
    ...
    // CLARINET — standing, clarinet vertical from mouth down, both hands on it.
    clarinet: `
    ...
    // FLUTE — standing, flute held horizontal across face.
    flute: `
    ...
    // BASS — standing tall on left, upright bass to the right.
    bass: `
    ...
    // VIOLIN — held under chin, bow horizontal across strings.
    violin: `
    ...
    // DRUMS — drummer seated, arms raised with sticks, snare in front.
    drums: `
    ...
    // VIBES — standing at vibraphone, mallets raised over horizontal bars.
    vibes: `
      <ellipse class="ln" cx="50" cy="20" rx="6.5" ry="7.5"></ellipse>
      <path class="ln" d="M 46 14 Q 50 10, 55 14"></path>
    ...
    // GUITAR — seated 3/4, guitar diagonal across body.
    guitar: `
    ...
    // ORGAN — figure seated at Hammond console, two manuals + drawbars.
    organ: `
      <line class="ln" x1="22" y1="98" x2="22" y2="108"></line>
      <line class="ln" x1="50" y1="98" x2="50" y2="108"></line>
      <line class="ln" x1="18" y1="98" x2="54" y2="98"></line>
    ...
    // VOICE — vocalist, mic raised to face.
    voice: `
    ...
    // REST — figure at rest, arms folded, no instrument. Used when primary
    // instrument is unknown. Small question-mark accent above head.
    rest: `
  ...
  // ─────────── Instrument string → figure key ───────────
  // Handles common variants. Unknown → 'rest'.
  function figKey(inst) {
    if (!inst || inst === '—' || inst === '') return 'rest';
    const i = String(inst).toLowerCase();
    // Brass
    if (i.includes('trombone')) return 'trombone';
    if (i.includes('trumpet') || i.includes('cornet') || i.includes('flugel')) return 'trumpet';
    if (i.includes('tuba') || i.includes('horn')) return 'trumpet';
    // Woodwinds (clarinet/oboe → clarinet, flute/piccolo → flute, sax variants → sax)
    if (i.includes('clarinet') || i.includes('oboe') || i.includes('bassoon')) return 'clarinet';
    if (i.includes('flute') || i.includes('piccolo')) return 'flute';
    if (i.includes('sax')) return 'sax';
    // Strings (violin family → violin, bass family → bass, plucked → guitar)
    if (i.includes('violin') || i.includes('fiddle') || i.includes('viola')) return 'violin';
    if (i.includes('cello') || i.includes('contrabass') || i.includes('bass')) return 'bass';
    if (i.includes('guitar') || i.includes('banjo') || i.includes('mandolin') || i.includes('ukulele')) return 'guitar';
    if (i.includes('harp')) return 'violin'; // closest visual mass
    // Keys
    if (i.includes('organ') || i.includes('hammond')) return 'organ';
    if (i.includes('piano') || i.includes('keys') || i.includes('keyboard') || i.includes('rhodes') || i.includes('wurlitzer')) return 'piano';
    // Percussion
    if (i.includes('vib') || i.includes('marimba') || i.includes('xylo')) return 'vibes';
    if (i.includes('drum') || i.includes('percussion') || i.includes('cymbal')) return 'drums';
    // Voice
    if (i.includes('voc') || i.includes('voice') || i.includes('sing') || i.includes('scat')) return 'voice';
    return 'rest';
  }

  // ─────────── Stable 3-digit serial from name ───────────
  function serialFor(name) {
    if (!name) return '000';
    let h = 0;
    const s = String(name);
    for (let i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) >>> 0;
    return String(h % 999).padStart(3, '0');
  }

  // ─────────── Initials helper ───────────
  // 1–2 character monogram for the corner mark. Skips common particles
  // ("van", "de", "of", "the") so "Van der Rohe" → "VR" not "VD".
  const SKIP = new Set(['van','von','de','del','della','di','da','du','la','le','of','the','and','y','el']);
  function initialsOf(name) {
    if (!name) return '';
    const parts = String(name).replace(/[.,'"]/g, '').split(/\s+/).filter(p => p && !SKIP.has(p.toLowerCase()));
    if (parts.length === 0) return '';
    if (parts.length === 1) return parts[0].slice(0, 2).toUpperCase();
    return (parts[0][0] + parts[parts.length - 1][0]).toUpperCase();
  }

  // ─────────── NoPhotoMark React component ───────────
  // Renders a .duo3-mark overlay containing the appropriate instrument figure
  // PLUS a small monogram in the bottom-right corner so each musician's tile
  // is unique. Place inside a .duo3 (which provides --duo-lo/hi and the
  // duotone field). Pass `name` for the monogram; omit to render figure only.
  function NoPhotoMark(props) {
    const { inst, name, className } = props || {};
    const key = figKey(inst);
    const inner = FIG_LIB[key] || FIG_LIB.rest;
    const ini = name ? initialsOf(name) : '';
    return React.createElement('div', {
      className: 'duo3-mark ' + (className || ''),
      'aria-hidden': 'true',
    },
      ...
      ini && React.createElement('span', { className: 'duo3-mark-ini' }, ini)
    );
  }

  Object.assign(window, { FIG_LIB, figKey, serialFor, initialsOf, NoPhotoMark });
})();
