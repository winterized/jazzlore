...
// Duo3 — duotone tile primitive.
// When `inst` is supplied, renders the editorial-line "no-photo" mark for
// that instrument (piano/trumpet/sax/bass/drums/guitar/trombone/clarinet/
// flute/violin/vibes/organ/voice/rest). Falls back to corner initials when
// `inst` is omitted. Unknown instrument string → "figure at rest" mark.
// Pass `initials={false}` to render an empty duotone (used for record cover
// placeholders).
function Duo3({ name, inst, style, initials = true, big, children, className = '' }) {
  const [lo, hi] = duotoneFor(name);
  const hasMark = inst !== undefined && inst !== null;
  return (
