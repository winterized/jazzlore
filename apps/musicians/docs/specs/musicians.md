...
- **e2e**: `musicians-pwa.spec.ts` (SW install/activate/control, manifest, shell-only precache), `musicians-offline.spec.ts` (banner + failed-nav visuals, light+dark), `musicians-safe-area.spec.ts` (3-device structural check). Chromium for the offline/safe-area specs.
