...
| **Waking / error** | BFF `503 {status:"waking"}` → calm "waking up" state with cached fallback names + retry countdown. Neo4j unreachable → calm error tone ("We missed a beat." + a technical small-print line), cached fallback names. Never a raw error. The hard-error **Try again** button is throttled on an escalating cooldown (first retry free, then 1s/2s/4s, capped 5s; resets to zero on a successful load) so a struggling backend isn't hammered — see `features/status/retryBackoff.ts`. |
