# Jazzlore Musicians — figures source (verbatim)

The 14 figures below are extracted **verbatim** from
`apps/musicians/src/components/noPhotoFigures.tsx` (`FIG_LIB` entries).
Each block is the inner SVG markup for one figure — what sits inside the
runtime wrapper:
...
When you redraw a figure, output the same shape — `line` / `path` /
`circle` / `ellipse` / `rect` elements carrying `class="ln" | "dot" |
"keyfill"`, no inline `stroke` / `fill` / `stroke-width`. The full
contract lives in `constraints.md` next to this file.
...
Standing facing right; slide extends forward, bell behind player.
...
Standing, sax body J-curves down across torso.
...
Standing, clarinet vertical from mouth down, both hands on instrument.
...
Standing, flute held horizontal across face.
...
Standing tall on left, upright bass to the right.
...
Held under chin, bow horizontal across strings.
...
Seated 3/4, guitar diagonal across body.
...
Drummer seated, arms raised with sticks, snare in front.
...
Standing at vibraphone, mallets raised over horizontal bars.
...
Figure seated at Hammond console, two manuals + drawbars.
...
Figure at rest, arms folded, no instrument. Used when primary instrument is unknown. Small question-mark accent above the head.
