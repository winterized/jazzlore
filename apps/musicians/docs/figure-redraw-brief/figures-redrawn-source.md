# Jazzlore Musicians - figures source (B / Open, redrawn)

The 14 inner-SVG blocks below replace the matching `FIG_LIB` entries in
`apps/musicians/src/components/noPhotoFigures.tsx`. Each block is the markup
that sits inside the runtime wrapper:
...
Same contract as the original set: only `.ln / .dot / .keyfill` classes, no
inline `stroke` / `stroke-width` / `fill` / `style`, head at `y ~ 22`, feet
at `y 134`. See `constraints.md` for the full rules.

## piano

Pianist seated profile, both arms reaching forward to a horizontal keyboard.
...
Like piano, but with two stacked manuals + drawbar strip on top of the console.
...
Standing, head tilted back, horn extending up-right.
...
Standing facing right; bell + slide both forward, bell flare opens to the right.
...
Standing, sax body as two J-curves widening toward the bell on the right.
...
Standing, clarinet vertical from mouth to navel, one arm wraps to the lower grip.
...
Standing, flute held horizontal across face, body extending right.
...
Player standing tall on the left, large pear-shaped upright bass to the right.
...
Tucked under chin, scroll up-right, long bow drawn horizontal across the strings.
...
Seated 3/4 view; guitar diagonal across the lap, neck extending up-right.
...
Drummer seated, facing viewer, arms raised with sticks; snare in front.
...
Standing at vibraphone; both arms forward, mallets pointing down at the bars.
...
Vocalist standing, one arm raised holding a microphone capsule to the face.
...
Figure at ease, arms folded. Small `.dot` accent + `?` glyph above the head (unknown instrument).
