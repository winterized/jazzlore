...
- **Runtime:** a single **unified Cloudflare Worker** — static assets + `/api/*` fetch handler + a dynamic `/sitemap.xml` + per-request **SEO head injection** for `/musicians/:id` document navigations (HTMLRewriter): per-musician `<title>` (rewritten in place), `<meta name=description>` (the static shell one is **removed** so exactly one survives), OG + Twitter tags + image, `<link rel=canonical>` (encoded-colon canonical-id URL), and `<script type="application/ld+json">` schema.org **Person / MusicGroup** (`musicianJsonLd` in `worker/og.ts`). All injection is **best-effort/additive** — any Aura failure (incl. cold/`waking`) falls back to the untouched SPA shell, so a cold page may momentarily ship no SEO head. **Injection is gated on a document navigation** (`Sec-Fetch-Dest: document` or `Accept: text/html`) — a bare `curl` (no `Accept`) gets the plain shell; real Googlebot/browsers trigger it. SEO audit + the per-fix history live in `apps/musicians/docs/seo-audit-findings.md` (incl. §13 deployment-verification addendum). `wrangler.musicians.jsonc` at repo root has a Worker `main` entry + an `ASSETS` assets binding. **There is no Pages `functions/` directory and there will not be one.** Worker entry: `apps/musicians/worker/index.ts`.
- **Native iOS shell (Capacitor) + PWA:** Musicians is a real installable PWA (vite-plugin-pwa, **app-shell-only** precache — `/api/*` is never cached; offline browsing of data is NOT a goal) AND the 4th Capacitor iOS app (`jazzlore-ios`, `make musicians`). It is the **only Jazzlore app with a backend**, so the native shell (served from `capacitor://localhost`, no BFF) **must not** use relative `/api` — `bffGet` (`src/hooks/useMusicianData.ts`) targets the absolute origin `https://musicians.jazzlore.com` when `isNativeApp()`, and **`CapacitorHttp`** (enabled in the iOS `capacitor.config.ts`) routes `fetch` through native HTTP to bypass the WebView CORS (the BFF sends no CORS headers, out of scope to change). Relative/same-origin is unchanged in browser/PWA. A `200` with a null/non-object body now **rejects** (defensive — never crash consumers on `data.x`; that was the on-device blank screen). Offline UX: `OfflineBanner` + `WakingState` `offline` variant keyed on `navigator.onLine` at fetch-reject. See PRs #151/#152/#153 + jazzlore-ios #5 + the Capacitor-native-BFF reference memory.
- **Home-screen widget (iOS, "Musician of the Day"):** a `.systemSmall` WidgetKit widget lives in the **jazzlore-ios** repo (`MusicianWidget` target, merged PR #8) — Pattern (d): bundled data, **no runtime network**. Two touch-points in THIS repo: (1) the build-time content generator **`scripts/generate-widget-content.mjs`** (`pnpm widget:content`) that fetches the BFF and writes `musicians.json` + 512² portraits into the iOS widget's `Resources/` (curated tier via the LIGHT `/api/musicians/by-ids` — the heavy detail query times out for high-degree greats, issue #155 at build time); (2) the deep-link handler `src/components/DeepLinkHandler.tsx` + pure `src/lib/deepLink.ts` that route `jazzlore-musicians://musician/<id>` taps to `/musicians/:id` via `@capacitor/app`'s `appUrlOpen` (native-only, `isNativeApp()`-gated). Full design/regeneration/audit docs live in the iOS repo (`docs/widget-{design,content,license-audit}.md`); see the iOS-widget reference memory.
...
- **Explicit-projection gotcha (post-#155):** `detailCypher`'s per-record and per-collaborator projections are EXPLICIT property maps (`r{.id, .title, …}`), NOT `{.*}` spreads. A new property added to a node in Aura **silently does not reach the frontend** until you add it to the projection list — even though `r{.*}` would have carried it. The Records project hit this: prod returned `apple_album_url: null` until P4 added `apple_album_url`/`spotify_album_url`/`cover_art_attribution` to the list (`worker/cypher.ts`). When wiring any new Record/Musician field end-to-end, update BOTH the projection AND the mapper, and run the live-Aura smoke (`scripts/aura-smoke.ts`) to confirm it flows. See the explicit-record-projection reference memory.
- **Record ordering** (`detailCypher`) is a chronological discography timeline (studio-first → non-compilation/live → `release_year` ASC → title), NOT an importance ranking — no graph signal encodes record importance (see `docs/records-ordering-findings.md`). The frozen mapper never re-sorts; the Cypher `ORDER BY` (before the `collect`) is the single source of truth.
...
- `neo4j-driver`, a Pages `functions/` dir, SSR/SSG of the **body** (v1 accepts JS-hidden body; SEO is served via per-request **head** injection only — OG + canonical + JSON-LD, see Runtime above. Body SSR + `schema:MusicAlbum` on records remain deferred Tier-A/B items in the SEO audit).
