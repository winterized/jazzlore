// Sitemap source-of-truth for the scales app.
//
// scales.jazzlore.com is served by an assets-only Cloudflare Worker (no Worker
// `main`) — unlike musicians, whose Worker serves /sitemap.xml dynamically from
// Aura. So the scales sitemap is the committed static file `public/sitemap.xml`,
...
// this module there breaks the config. Instead, `scalesSitemap()` is the
...
//
// The crawlable surface is the 12 root pages (`/scales/<slug>`); every scale
// renders WITHIN a root page, so scales are not separate URLs. The paths are
...
const ORIGIN = 'https://scales.jazzlore.com'
...
export function scalesSitemapPaths(): string[] {
  return DEFAULT_ROOTS.map((root) => `/scales/${slugFromRoot(root)}`)
}
...
/** The complete scales sitemap document. */
export function scalesSitemap(): string {
  return buildSitemapXml(ORIGIN, scalesSitemapPaths())
}
