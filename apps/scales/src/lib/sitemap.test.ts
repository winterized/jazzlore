...
import { buildSitemapXml, scalesSitemap, scalesSitemapPaths } from './sitemap'
...
describe('scales sitemap', () => {
  it('emits one URL-safe path per default root', () => {
    const paths = scalesSitemapPaths()
    expect(paths.length).toBe(DEFAULT_ROOTS.length) // 12
    expect(paths).toContain('/scales/C')
    expect(paths).toContain('/scales/D-flat')
    expect(paths).toContain('/scales/F-sharp')
    // No raw accidentals (#, ♭) may leak into a URL.
    expect(paths.some((p) => p.includes('#') || p.includes('♭'))).toBe(false)
  })

  it('builds a well-formed urlset of absolute scales.jazzlore.com URLs', () => {
    const xml = scalesSitemap()
    expect(xml).toContain('<?xml version="1.0" encoding="UTF-8"?>')
    expect(xml).toContain(
      '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">',
    )
    expect(xml).toContain('<loc>https://scales.jazzlore.com/scales/C</loc>')
    expect(xml).toContain(
      '<loc>https://scales.jazzlore.com/scales/F-sharp</loc>',
    )
  ...
  // since scales has no runtime Worker) MUST match what the generator produces.
  // If DEFAULT_ROOTS changes, regenerate public/sitemap.xml from scalesSitemap().
  it('public/sitemap.xml matches the generated sitemap', () => {
    expect(normalize(committedSitemap)).toBe(normalize(scalesSitemap()))
  })
})
