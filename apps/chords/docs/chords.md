...
The grouped card list is a responsive grid: 1 column by default, **2 columns at `xl` (≥1280px)** so regular desktops get two cards per row. Each chord group is its own grid (`<ul>`), so it starts a new row with its section divider spanning full width above it; an odd group leaves one trailing empty cell (`align-items: start` — not filled/centered/stretched, by design; revisit only on user feedback). Sub-breakpoint rendering (<1280) is byte-identical to the prior single-column layout (MD5-gated at 390/1024/1279).

**Half-width chord card == full 640-viewport card.** At ≥1280 each card is half-width, which would otherwise crush the keyboard under the wide ≥768 internal layout. Instead a grid-scoped `@media (min-width:1280px)` override (keyed on the `.chord-card-grid` hook class on the group `<ul>`) plus the `inGrid` prop (drives `staffwidth` 170) make a half-width 1280 chord card render the exact 480–767 compact layout (mini-score on the header row, full-width keyboard below). The override is grid-scoped on purpose: the always-full-width `/collection/chords` page has no `.chord-card-grid` ancestor and never passes `inGrid`, so it keeps the wide layout at all widths.

Accepted/known: at 2-col the scroll-spy active chip biases to the right-column card of the topmost row past the threshold (load-state active chip becomes the 2nd chord) — coherent, now at ≥1280, presentational-only, not mitigated; the right-column-targets-its-own-chord contract is guarded in `tests/e2e/grid-layout.spec.ts`. The two base scroll-spy tests in `tests/e2e/chords.spec.ts` are deliberately pinned to 1024 (single-column) so they test the base mechanism, not the 2-col bias.

(History: first shipped at `2xl`/≥1536 — chosen to keep the 1280 baseline byte-identical — then moved to `xl`/≥1280 the same day per user request for two columns on regular desktops; the byte-identical region and the MD5/e2e boundary moved to 1279/1280 accordingly.)
