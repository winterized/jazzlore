 ...
 *  dedicated port (5183). The dev server (port 5173) intentionally has no
 *  service worker — see apps/scales/vite.config.ts `devOptions.enabled: false`
 *  — so these tests cannot run against it. The preview-server webServer entry
 *  in playwright.config.ts builds + serves the scales dist for this. */
const PREVIEW_BASE = 'http://localhost:5183'

test.describe('scales PWA — service worker + manifest', () => {
  ...
  test('/manifest.webmanifest is unchanged by the plugin (regression guard)', async ({
    ...
    expect(json.name).toBe('Jazzlore Scales')
