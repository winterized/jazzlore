 ...
 *  dedicated port (5184). The dev server (port 5174) intentionally has no
 *  service worker — see apps/chords/vite.config.ts `devOptions.enabled: false`
 *  — so these tests cannot run against it. The preview-server webServer entry
 *  in playwright.config.ts builds + serves the chords dist for this. */
const PREVIEW_BASE = 'http://localhost:5184'

test.describe('chords PWA — service worker + manifest', () => {
