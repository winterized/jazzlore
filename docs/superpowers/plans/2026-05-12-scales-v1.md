...
git commit -m "feat(lib): music helper — notes from root + curated scale"
```
...
- [ ] **Step 25.2: If bundle > 300 KB**, audit with rollup-plugin-visualizer or `pnpm build --report`. Likely culprit: pulling Tonal umbrella when modular packages would tree-shake better. Switch `@tonaljs/tonal` → `@tonaljs/note` (only `Note.transpose` is used in `src/lib/music.ts`).
...
- **Tonal name lookup:** `src/lib/music.ts` deliberately uses `Note.transpose` with semitone offsets — it does NOT depend on Tonal's scale dictionary names. If we ever need to validate against `ScaleType.all()` for cross-checking, that's a separate utility, not part of the core path. This keeps us insulated from Tonal version churn.
- **Tonal modular packages:** if bundle exceeds 300 KB gzipped, swap `@tonaljs/tonal` for `@tonaljs/note` in `package.json` and `src/lib/music.ts` imports — `Note.transpose` is the only Tonal call we make.
